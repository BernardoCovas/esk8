...
void test_e_ride_uart_msg_parse()
{
    ...
    errCode = e_ride_uart_msg_parse(valid_msg, 10, &msg);
    TEST_ASSERT_EQUAL(E_RIDE_SUCCESS, errCode);
    if(errCode == E_RIDE_SUCCESS)
        e_ride_uart_msg_free(msg);

    errCode = e_ride_uart_msg_parse(invalid_msg_pldlen, 10, &msg);
    TEST_ASSERT_EQUAL(E_RIDE_UART_MSG_ERR_INVALID_PLDLEN, errCode);
     if(errCode == E_RIDE_SUCCESS)
        e_ride_uart_msg_free(msg);

    errCode = e_ride_uart_msg_parse(invalid_msg_cksum, 10, &msg);
    TEST_ASSERT_EQUAL(E_RIDE_UART_MSG_ERR_INVALID_CHKSUM, errCode);
    if(errCode == E_RIDE_SUCCESS)
        e_ride_uart_msg_free(msg);

    errCode = e_ride_uart_msg_parse(invalid_msg_no_hdr, 10, &msg);
    TEST_ASSERT_EQUAL(E_RIDE_UART_MSG_ERR_NO_HEADER, errCode);
    if(errCode == E_RIDE_SUCCESS)
        e_ride_uart_msg_free(msg);

    errCode = e_ride_uart_msg_parse(invalid_msg_buffer, 10, &msg);
    TEST_ASSERT_EQUAL(E_RIDE_UART_MSG_ERR_INVALID_BUFFER, errCode);
    if(errCode == E_RIDE_SUCCESS)
        e_ride_uart_msg_free(msg);
}
