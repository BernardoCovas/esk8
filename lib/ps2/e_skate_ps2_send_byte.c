...
void clear_handle(e_skate_ps2_handle_t* ps2Handle)
{
    uint8_t c_pin = ps2Handle->ps2Config.gpioConfig.clockPin;
    uint8_t d_pin = ps2Handle->ps2Config.gpioConfig.dataPin;
...
)
{
    ...
    ps2Handle->txPkt.newParity  = e_skate_ps2_get_parity(byte);
    ...
    if (!ulTaskNotifyTake(
            pdTRUE,
            E_SKATE_PS2_PACKET_TIMEOUT_MS * 20 / portTICK_PERIOD_MS))
        {
